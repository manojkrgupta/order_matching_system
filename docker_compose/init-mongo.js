...
db.account.insertOne({'account_id': 'a5', 'name': 'Name Four' , 'broker': 'Prostoc', 'CDSL' : '12345', 'segment': ['Equity', 'Derivatives']})
db.account.insertOne({'account_id': 'a6', 'name': 'Name Four' , 'broker': 'Zerodha', 'CDSL' : '12346', 'segment': ['Equity', 'Derivatives']})

// Stock Order Table
db.createCollection('stock.order.apple');
db.createCollection('stock.order.amazon');
db.createCollection('stock.order.zoomato');
db.createCollection('stock.order.ibm');
...
db.stock.order.ibm.ensureIndex({'order_id': 1}, {unique: true}); // create unique constraint
...
db.createCollection('stock.trade.ibm');
...
db.stock.trade.ibm.ensureIndex({'trade_id': 1}, {unique: true}); // create unique constraint
